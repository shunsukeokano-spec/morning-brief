...
<div class="note-section">
  ...
  {"" if worker_url else '<div class="note-not-configured">⚙ Worker URL not configured — see docs/SETUP.md</div>'}
  <textarea class="note-textarea" id="note-input" placeholder="気になったこと・質問を一行書く。明日のブリーフに反映される。" {"" if worker_url else "disabled"}></textarea>
  <div class="note-row">
    <button class="note-save" id="note-save" onclick="saveNote()" {"" if worker_url else "disabled"}>Save to Tomorrow's Brief</button>
...
<script>
const WORKER_URL = {repr(worker_url)};
...
async function saveNote() {{
  ...
  try {{
    ...
    if (res.ok) {{
      ...
      setStatus('err', 'Save failed (' + res.status + ') — try again');
    }}
  }} catch(e) {{
